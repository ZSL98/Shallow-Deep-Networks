...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ee_optimizer = torch.optim.SGD(ee.parameters(), args.lr,
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            ee.load_state_dict(checkpoint['state_dict'])
            ee_optimizer.load_state_dict(checkpoint['optimizer'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        adjust_learning_rate(ee_optimizer, epoch, args)

        # train for one epoch
        train(train_loader, ee, backbone, criterion, ee_optimizer, epoch, args)

        # evaluate on validation set
        acc1 = validate(val_loader, ee, backbone, criterion, args)
...
#     print(features_in_hook[0][0].shape)
    ...
    # new_m = models._utils.IntermediateLayerGetter(model, {'layer1': 'feat1', 'layer2': 'feat2'})
